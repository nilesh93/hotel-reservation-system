...
Left JOIN  HALLS B ON B.hall_id = A.hall_id
LEFT JOIN  CUSTOMER C ON A.cus_id = A.cus_id
...
where A.room_type_id = ''"));
    ...
    public function getReservationDates(Request $request){
        ...
        $rooms = DB::select(DB::raw("Select * from  ROOMS C
...
(select count(*) from  ROOMS F where F.status = 'AVAILABLE' AND F.room_type_id = B.room_type_id) as all_rooms,
IFNULL((select SUM(A.count) as booked 
from  RES_RMTYPE_CNT_RATE A
where A.room_reservation_id IN (
 Select  J.room_reservation_id from  ROOM_RESERVATION J 
...
from HALLS A
 ...
 "));
    ...
    public function showBlocks(Request $request){
        ...
        $check = DB::select(DB::raw("select * from  ROOM_RESERVATION A
